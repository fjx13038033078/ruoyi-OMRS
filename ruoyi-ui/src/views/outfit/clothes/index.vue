  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="80px">
      ...
      <el-form-item label="收藏" prop="isFavorite">
        <el-select v-model="queryParams.isFavorite" placeholder="是否收藏" clearable>
          ...
          <el-card class="clothes-card" shadow="hover" :body-style="{ padding: '0' }">
            <div class="card-image" @click="handleView(item)">
              ...
              <div class="favorite-icon" @click.stop="handleToggleFavorite(item)">
                <i :class="item.isFavorite === '1' ? 'el-icon-star-on' : 'el-icon-star-off'"></i>
              </div>
            </div>
            <div class="card-content">
              ...
              <div class="card-actions">
                ...
                <el-button type="text" size="mini" icon="el-icon-edit" @click="handleUpdate(item)" v-hasPermi="['outfit:clothes:edit']">编辑</el-button>
                <el-button type="text" size="mini" icon="el-icon-delete" @click="handleDelete(item)" v-hasPermi="['outfit:clothes:remove']">删除</el-button>
              </div>
            </div>
          </el-card>
        </el-col>
    ...
    <!-- 表格视图 -->
    <el-table v-if="viewMode === 'table'" v-loading="loading" :data="clothesList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="穿着次数" align="center" prop="wearCount" width="80" />
      <el-table-column label="收藏" align="center" width="60">
        <template slot-scope="scope">
          <i 
            :class="scope.row.isFavorite === '1' ? 'el-icon-star-on' : 'el-icon-star-off'" 
            :style="{ color: scope.row.isFavorite === '1' ? '#f7ba2a' : '#c0c4cc', cursor: 'pointer', fontSize: '18px' }"
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="180">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)" v-hasPermi="['outfit:clothes:edit']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)" v-hasPermi="['outfit:clothes:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="衣物详情" :visible.sync="viewOpen" width="650px" append-to-body>
      <el-descriptions :column="2" border v-if="viewData">
        ...
        <el-descriptions-item label="穿着次数">{{ viewData.wearCount || 0 }}次</el-descriptions-item>
        <el-descriptions-item label="最近穿着">{{ viewData.lastWearDate || '-' }}</el-descriptions-item>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="handleWear(viewData)" v-hasPermi="['outfit:clothes:edit']">记录穿着</el-button>
...
export default {
  ...
  methods: {
    ...
    handleToggleFavorite(row) {
      const newFavorite = row.isFavorite === '1' ? '0' : '1';
      toggleFavorite(row.id, newFavorite).then(() => {
        row.isFavorite = newFavorite;
        this.$modal.msgSuccess(newFavorite === '1' ? '已收藏' : '已取消收藏');
      });
    },
    /** 记录穿着 */
    handleWear(row) {
